...
| `GET /api/status` | — | `{ "busy": bool, "provider": "anthropic"\|"openai", "runs_started_today": int, "daily_run_limit": int }` | Reflects live `RunManager` state; used by the dashboard to disable the trigger form while busy or over quota |
